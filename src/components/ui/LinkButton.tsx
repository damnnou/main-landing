...
export function LinkButtonWithArrow({ children, href }: { children: React.ReactNode; href: string }) {
    return (
        <Link target="_blank" href={href}>
            <button className="flex group relative items-center w-fit gap-4 bg-background hover:bg-transparent hover:transition-colors hover:delay-100 rounded-full px-5 p-2.5">
                <span className="text-button mr-10 text-black group-hover:text-white transition-colors ease-in-out duration-200 z-10">
                    {children}
                </span>
                <div className="radial-gradient transition-all duration-200 ease-in-out right-[11px] group-hover:right-0  absolute rounded-full group-hover:w-full group-hover:h-full w-[36px] h-[36px]"></div>
...
export function LinkButton({ icon, href, alt }: { icon: string; href: string; alt: string }) {
    return (
        <Link target="_blank" href={href}>
            <button className="flex items-center justify-center group w-[56px] h-[56px] relative gap-4 bg-background hover:bg-transparent hover:transition-colors hover:delay-100 rounded-full ">
                <img
